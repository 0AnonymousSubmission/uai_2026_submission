# type: ignore
"""
K-Fold Cross-Validation runner for TN-ALS (Tensor Network Alternating Least Squares).
...
def extract_tn_metrics(tn) -> dict:
    """Extract metrics from tensor network."""
    metrics = {
        "bond_dims": {},
...
def run_single_fold(
    ...
    """
    Run a single fold of k-fold cross-validation.
    ...
    try:
        ...
        initial_metrics = extract_tn_metrics(tn)
        initial_bond_dims = initial_metrics.get("bond_dims", {})
        initial_n_params = initial_metrics.get("n_parameters", 0)
        ...
        for epoch in range(n_epochs):
            ...
            if epoch >= warmup_epochs and decay != 1.0:
        ...
        if best_model_state is not None:
            for tag, data_tensor in best_model_state.items():
                tn.mu[tag].modify(data=data_tensor)
        ...
        final_metrics = extract_tn_metrics(tn)
        ...
        result = {
            ...
            "final_n_parameters": final_metrics["n_parameters"],
            "success": True,
            "singular": False,
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Run K-Fold Cross-Validation for TN-ALS across all datasets"
    )
    ...
    parser.add_argument("--bond-prior-alpha", type=float, default=1.0, help="Bond prior alpha")
    parser.add_argument("--added-bias", action="store_true", help="Add bias term (ignored if --specs-file)")
    
    parser.add_argument("--n-epochs", type=int, default=50, help="Max epochs")
    ...
    parser.add_argument("--decay", type=float, default=1.0, help="Bond decay factor")
    ...
    parser.add_argument("--output-dir", type=str, default="results/kfold_als",
                        help="Output directory")
    parser.add_argument("--tracker", type=str, default="file",
                        choices=["file", "aim", "both", "none"], help="Tracker backend")
    parser.add_argument("--tracker-dir", type=str, default="runs_kfold",
    ...
    print("=" * 70)
    print("K-FOLD CROSS-VALIDATION FOR TN-ALS")
    print("=" * 70)
