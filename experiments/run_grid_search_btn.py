...
"""
Grid search experiment runner for BTN (Bayesian Tensor Network).
...
def run_single_experiment(
    ...
    trimming_threshold = params.get("trimming_threshold", 0.2)
    ...
    try:
        ...
        for epoch in range(n_epochs):
            trim_start = warmup_epochs + trim_every
            ...
            if trim_every is not None and epoch >= trim_start:
                if trim_epoch % trim_every == 0 and not btn.has_pending_soft_trims():
...
def main():
    ...
    print(f"  Task: regression (BTN only)")
