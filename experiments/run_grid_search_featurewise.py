...
from experiments.dataset_loader import load_featurewise_polynomial_dataset, load_dataset, expand_features_polynomial
...
"""
Grid search for BTN with featurewise polynomial expansion.
Each site = one feature, phys_dim = degree+1, X shape [n_samples, n_features, phys_dim].
"""
...
def run_single_experiment(
    ...
    """Run a single BTN experiment with given parameters."""
    ...
    trim_every = params.get("trim_every", None)
    trimming_threshold = params.get("trimming_threshold", 0.95)
    ...
    phys_dim = data["X_train"].shape[2]
    params["L"] = n_features

    try:
        model = create_btn_model(params, phys_dim)
        mu_tn = model.tn
        ...
        X_train_list = [data["X_train"][:, i, :] for i in range(n_features)]
        X_val_list = [data["X_val"][:, i, :] for i in range(n_features)]
        X_test_list = [data["X_test"][:, i, :] for i in range(n_features)]
        ...
        for epoch in range(n_epochs):
            for node_tag in nodes:
                ...
                btn.trim_zero_variance()
                btn.update_mu_node(node_tag)

            if epoch >= warmup_epochs:
                excluded_bonds = btn.get_soft_trim_excluded_bonds()
            ...
            if trim_every is not None and epoch >= warmup_epochs:
                ...
                if epochs_since_warmup % trim_every == 0 and not btn.has_pending_soft_trims():
                    if trim_method == "variance":
                        variance_thresh = 1.0 - trimming_threshold
                        btn.trim_bonds_by_variance(
                            threshold=variance_thresh, relative=True, verbose=False
                        )
                    elif trim_method == "soft_relevance":
                        marked = btn.prepare_soft_trim_bonds(verbose=False)
                        if marked > 0:
                            soft_trim_started_epoch = epoch
                    else:
            ...
            tracker.log_summary(
                {
                    ...
                    "n_parameters": sum(t.data.numel() for t in btn.mu.tensors),
                }
            )
...
def main():
    parser = argparse.ArgumentParser(description="Run BTN grid search experiments")
    parser.add_argument("--config", type=str, required=True, help="Path to JSON config file")
    parser.add_argument(
        "--output-dir", type=str, required=False, help="Output directory for results"
    )
    parser.add_argument(
        "--tracker",
        type=str,
    ...
    experiment_plan, metadata = create_experiment_plan(config, method="BTN")
    ...
    if "polynomial" in config:
        ...
        print(f"\nGenerating synthetic featurewise polynomial dataset")
        raw_data, dataset_info = load_featurewise_polynomial_dataset(
            n_samples=poly_config.get("n_samples", 1000),
            n_features=poly_config.get("n_features", 5),
            degree=poly_config.get("degree", 2),
            noise_std=poly_config.get("noise_std", 0.1),
            seed=poly_config.get("seed", 42),
            device="cpu",
        )
        raw_data = move_data_to_device(raw_data)
        use_raw_uci = False
    else:
        dataset_name = config["dataset"]
        print(f"\nLoading UCI dataset: {dataset_name} (degree from L param)")
        raw_data, dataset_info = load_dataset(dataset_name, device="cpu", cap=config.get("feature_cap", 20))
        raw_data = move_data_to_device(raw_data)
        use_raw_uci = True

    print(f"  Task: regression (BTN featurewise)")
    print(f"  Device: {DEVICE}")
    print(f"  Features (sites): {dataset_info['n_features']}")
    print(f"  Train: {dataset_info['n_train']} samples")
    print(f"  Val: {dataset_info['n_val']} samples")
    print(f"  Test: {dataset_info['n_test']} samples")
    print(f"  Note: L parameter = polynomial degree (phys_dim = L+1)")
    ...
    for idx, experiment in enumerate(experiment_plan, 1):
        ...
        if use_raw_uci:
            degree = params["L"]
            data_copy = {k: v.clone() for k, v in raw_data.items()}
            data = expand_features_polynomial(data_copy, degree=degree, device=str(DEVICE))
            exp_dataset_info = {
                **dataset_info,
                "phys_dim": degree + 1,
                "degree": degree,
