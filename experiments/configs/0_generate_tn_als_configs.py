#!/usr/bin/env python3
"""
Generate experiment config files for TN-ALS on all UCI regression datasets.

Supports CPD with separate rank parameter (cpd_rank) that maps to bond_dim for CPD models.
"""
...
def create_config(
    ...
    return {
        ...
        "tracker": tracker,
        "output": {
            "results_dir": f"{base_dir}/als_{dataset_name}_{model}_{bias_tag}",
            "save_models": False,
...
def main():
    ...
    for dataset_name, _ in UCI_REGRESSION_DATASETS:
        ...
        for added_bias in [False, True]:
            ...
            configs = [
                (f"als_{dataset_name}_mpo2_{bias_tag}.json", create_config(dataset_name, "MPO2", added_bias=added_bias)),
                (
                    f"als_{dataset_name}_lmpo2_{bias_tag}.json",
                    create_config(
                        dataset_name,
                        "LMPO2",
                ...
                (f"als_{dataset_name}_cpd_{bias_tag}.json", create_config(dataset_name, "CPD", added_bias=added_bias)),
            ]

            if include_btt:
                configs.append((f"als_{dataset_name}_btt_{bias_tag}.json", create_config(dataset_name, "BTT", added_bias=added_bias)))
